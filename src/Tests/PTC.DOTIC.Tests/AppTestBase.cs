...
using PTC.DOTIC.Migrations.Seed.Tenants;
using PTC.DOTIC.MultiTenancy;
using PTC.DOTIC.Tests.TestDatas;
...
namespace PTC.DOTIC.Tests
{
    ...
    /// Provides methods to easily work with <see cref="DOTICDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<DOTICTestModule>
    {
        ...
        protected void UsingDbContext(Action<DOTICDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<DOTICDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<DOTICDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<DOTICDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<DOTICDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DOTICDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<DOTICDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DOTICDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<DOTICDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<DOTICDbContext, Task<T>> func)
        {
            T result;
